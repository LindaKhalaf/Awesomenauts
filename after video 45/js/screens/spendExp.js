	...
	onResetEvent: function() {	
		...
		me.game.world.addChild(new (me.Renderable.extend({
			...
			draw: function(renderer){
				...
				this.font.draw(renderer.getContext(), "F1: Increase Gold Production Current Level: " + me.save.exp1.toString() + " Cost: " + exp1cost, this.pos.x, this.pos.y + 100);
			...
			this.handler = me.event.subscribe(me.event.KEYDOWN, function(action, keyCode, edge){
				if(action === "F1"){
					if(game.data.exp >= exp1cost){
						me.save.exp1 = game.data.exp1 += 1;
						game.data.exp -= exp1cost;
						me.state.change(me.state.PLAY);
					}else{
						console.log("not enough experiance");
					}
